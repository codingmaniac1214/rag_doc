...
#This was made to fix chunks and index before products were introduced
...
#             logging.debug(f"Loaded FAISS index from {index_path}")
#         else:
#             logging.debug("No existing FAISS index found. Will create a new one if needed.")
#     except Exception as e:
#         logging.warning(f"Failed to load FAISS index: {e}. Will create a new one later if needed.")

#     # Load or initialize chunk mapping
#     chunk_mapping = {"chunks": []}
#     existing_chunks = []
#     try:
#         if os.path.exists(chunk_mapping_path):
...
#                     if isinstance(loaded_data, list):
#                         logging.warning("Old format detected in chunk_mapping.json. Converting to dict format.")
#                         converted_chunks = []
#                         for chunk in loaded_data:
#                             if isinstance(chunk, list) and len(chunk) >= 4:
#                                 converted_chunks.append({
...
#                         chunk_mapping = loaded_data
#                     existing_chunks = chunk_mapping.get("chunks", [])
#                     logging.debug(f"Loaded {len(existing_chunks)} chunks from chunk_mapping.json")
#                 else:
#                     logging.warning(f"{chunk_mapping_path} is empty. Starting fresh.")
#     except Exception as e:
#         logging.warning(f"Failed to read or parse chunk_mapping.json: {e}. Starting fresh.")
...
def preprocess_directory(input_dir, text_dir, metadata_dir, chunks_dir=None):
   ...
   logging.debug(f"Preprocessing input_dir: {input_dir}, text_dir: {text_dir}, metadata_dir: {metadata_dir}, chunks_dir: {chunks_dir}")
   config = load_config('config.yaml')
   product = os.path.basename(os.path.dirname(input_dir))
   index_path = os.path.join(config['base_embeddings_dir'], product, "faiss_index")
   chunk_mapping_path = os.path.join(config['base_embeddings_dir'], product, "chunk_mapping.json")
   chunk_size = config['chunk']['size']
   chunk_overlap = config['chunk']['overlap']
   chunks_dir = chunks_dir or text_dir
   ...
   if os.path.exists(index_path) and os.path.exists(chunk_mapping_path):
       try:
           ...
           logging.error(f"Failed to load FAISS index or chunk mapping: {e}. Initializing new index.")
           index = None
           chunk_mapping = {"chunks": []}
   else:
       logging.debug(f"No FAISS index or chunk mapping found for product '{product}'. Initializing new index.")
       index = None
       chunk_mapping = {"chunks": []}
           ...
           if text_path and meta_path and chunk_path:
               ...
               if embeddings is not None:
                   if index is None:
                       dimension = embeddings.shape[1]
                       index = faiss.IndexFlatL2(dimension)
                       logging.debug(f"Initialized new FAISS index with dimension {dimension} for product '{product}'")
                   index.add(embeddings)
                   logging.debug(f"Added embeddings for {file_path}")
   ...
   try:
       os.makedirs(os.path.dirname(index_path), exist_ok=True)
       faiss.write_index(index, index_path)
       logging.debug(f"Saved FAISS index to {index_path}")
   except Exception as e:
